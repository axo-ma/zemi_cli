...
### Виртуальное окружение компонента

Для работы с Python в этом компоненте всегда используй виртуальное окружение
`@comp/.venv`, созданное поверх WinPython `@inst/_pythons/WPy64-312101/python`.

- Запускай Python через `@comp/.venv/Scripts/python.exe`.
- Устанавливай и проверяй Python-зависимости только в `@comp/.venv`.
- Запускай тесты, скрипты и проверки импортов через Python из `@comp/.venv`.
- Не используй напрямую системный Python или базовый Python из WinPython, если
  задача явно не требует работы с самой базовой средой.
...
- Каталог, который должен быть отдельным корнем VS Code Multi-root Workspace,
  содержит файл-маркер `.zemiworkroot`. Для ZEMI Component отдельный маркер не
  нужен: достаточно `.zemicomp`.
