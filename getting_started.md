...
Команда выполняет `git clone --recurse-submodules` репозитория шаблона
`https://github.com/axo-ma/zemi_component_template.git` непосредственно в новый
каталог компонента. После клонирования она удаляет `origin` шаблона и создаёт
обязательный файл-маркер `.zemicomp`.

Затем можно указать URL заранее созданного пустого Git-репозитория. Команда
проверит, что репозиторий доступен и не содержит веток или тегов, после чего
добавит его как новый `origin`. Оставьте URL пустым, чтобы работать с локальным
компонентом без remote.

Команда не создаёт commit и не отправляет изменения автоматически. Сначала
проверьте и настройте компонент, затем выполните обычные команды Git:
...
Последняя команда нужна только в варианте с настроенным `origin`.

## 5. Создание Python-окружения компонента

После распаковки WinPython перейдите в каталог ZEMI Component и выполните:
...
Никакие зависимости проекта команда не устанавливает. Повторный запуск безопасно
завершается, только если существующий `.venv` уже связан с ожидаемым WinPython и
имеет доступ к его `site-packages`; другое окружение команда не изменяет.
